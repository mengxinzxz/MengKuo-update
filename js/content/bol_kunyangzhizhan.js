        ...
        chooseCharacterBefore() {
            ...
            const changeFunction = {
                ...
                lib: {
                    character: (() => {
                        const character = {
                            gw_liuxiu: ['male', 'qun', 10, []],
                            gw_wangmang: ['male', 'qun', 42, ['mashu', 'gw_jianqu']],
                            gw_shibing: ['male', 'qun', 1, ['mashu']],
                        };
                    ...
                    skill: {
                        ...
                        _gw_die: {
                            ...
                            superCharlotte: true,
                            ruleSkill: true,
                            trigger: { player: ['dieBefore', 'restEnd'] },
                            filter(event, player, name) {
                                if (player.identity !== 'cai') return false;
                                return name === 'restEnd' || player.hasMark('_gw_xinnian');
                            },
                            direct: true,
                            priority: 15,
                            async content(event, trigger, player) {
                                if (trigger.name === 'die') {
                                    ...
                                    trigger.restMap = { type: 'round', count: 1 };
                                    trigger.includeOut = true;
                                }
                                else {
                                    await player.draw(7);
                                    const next = game.createEvent('_gw_xinnian');
                                    next.player = player;
                        ...
                        _gw_win: {
                            ...
                            async content(event, trigger, player) {
                                ...
                                if (targets.length) {
                                    player.line(targets);
                                    targets.forEach(target => player.throwEmotion(target, 'egg'));
                                }
                    ...
                    element: {
                        player: {
                            ...
                            dieAfter() {
                                var that = this;
                                if (that.identity == 'zhu') that.chat('我焯，挂');
            ...
            game.players.forEach((i, index) => {
                ...
                else if (index === 3) i.identity = 'zhu';
                else i.identity = 'zhong';
                i.init({ cai: 'gw_liuxiu', zhu: 'gw_wangmang', zhong: 'gw_shibing' }[i.identity]);
                i.showIdentity();
            });
            game.showIdentity();
        },
