    ...
    init() {
                ...
                hezongkangqin_player_init: {
                    ...
                    content: function () {
                        ...
                        for (const name in lib.character) {
                            ...
                            var skillsx = get.character(name).skills.slice();
                        ...
                        if (_status.ol_hezongkangqin_name) {
                            var name;
                            switch (_status.ol_hezongkangqin_name) {
                                case '变法者': {
                                    name = '变法图强';
                        ...
                        var list = lib.hezongkangqin.skills.randomGets(15);
                        for (var i = 0; i < list.length; i++) {
                            list[i] = ['', '', 'skillCard_' + list[i]];
                        }
                        event.current.chooseButton(['选择要获得的技能', [list, 'vcard']], true, [0, 2 + Math.floor((event.current._kangqinLevel - 1) / 2)]);
                        'step 4'
        ...
        for (var i in pack.character) {
            ...
            if (lib.device || lib.node) {
                ...
                pack.character[i][4].push('db:extension-活动萌扩/image:' + i + '.jpg');
            }
        }
        ...
        chooseCharacterBefore: function () {
            ...
            game.chooseCharacter = function () {
                ...
                next.setContent(async function () {
                    ...
                    ], true).set('onfree', true).forResult();
                    if (!result.bool) game.reload();
                    const choice = result.links[0];
                    _status.ol_hezongkangqin_name = choice.name;
                    let players = choice.players, item = players.filter(i => i.isMe).randomGet();
