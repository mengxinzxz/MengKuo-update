                ...
                game: {
                    checkResult() {
                        switch (game.players.length) {
                            case 1:
                                game.over(game.me === game.players[0]);
                                break;
                            case 2:
                                if (game.players.includes(game.me)) {
                    ...
                    chooseCharacter() {
                        ...
                        next.setContent(async function (event, trigger, player) {
                            ...
                            }), list1 = list.randomGets(5);
                            ui.arena.classList.add('choose-character');
                            const dialog = event.dialog = ui.create.dialog('请选择您的武将', 'hidden');
                            ...
                            game.players.forEach(target => {
                                if (target !== player) target.init(_status.characterlist.randomRemove(1)[0]);
                            });
                        ...
                        player: {
                            ...
                            getFriends(func, includeDie, includeOut) {
                                ...
                                return game[method](function (current) {
                                    if (func === true && current == player) return false;
                            ...
                            dieAfter2(source) {
                                if (source) {
                                    let num = 1;
                                    if (_status.luanwu_result && _status.luanwu_result == 'luanwu2') num *= 2;//重赏
                                    source.draw(3 * num);
